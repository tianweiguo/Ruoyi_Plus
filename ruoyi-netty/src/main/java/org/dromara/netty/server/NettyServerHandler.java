...
import org.dromara.netty.common.Command;
import org.dromara.netty.common.MessageBase;
...
//	private final static List<String> reqIdCache = new ArrayList<>();
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		try {
    		...
    		if(msgBase.getCmdType() == Command.CommandEnum.AUTH.value){
				log.info("client auth>>> "+clientId);
				Attribute<String> attr = ctx.attr(clientInfo);
				attr.set(clientId);
				channelCache.put(clientId, ctx.channel());

				ctx.writeAndFlush(createData(clientId, Command.CommandEnum.AUTH_BACK, "auth_back>>> auth success"));

			}else if(msgBase.getCmdType() == Command.CommandEnum.PING.value){
				//处理ping消息
				ctx.writeAndFlush(createData(clientId, Command.CommandEnum.PONG, "pong>>> This is pong data"));
			}else{
				if(JSONUtil.isJson(msgBase.getData())){
					...
					if(!StringUtils.isEmpty(rid)){
						...
						otherCh.writeAndFlush(
								createData(rid, Command.CommandEnum.PUSH_DATA, msgBase.getData())
							);
					}
				}
			}
	...
	private TextWebSocketFrame createData(String clientId, Command.CommandEnum cmdEnum, String data){
		MessageBase msg = new MessageBase();
		msg.setClientId(clientId);
		msg.setCmdType(cmdEnum.value);
		msg.setData(data);
